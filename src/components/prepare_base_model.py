...
class Prepare_base_model:
    def __init__(self, prepare_base_model_config: PrepareBaseModelConfig):
        try:
            ...
            logging.error(
                f"Prepare base model object creation interrupted due to {CustomException(e,sys)}"
            )
    ...
    def get_base_model(self):
        try:
            ...
            self.save_model(
                path=self.prepare_base_model_config.base_model_path, model=self.model
            )
        except Exception as e:
            logging.error(
                f"getting base model interrupted due to {CustomException(e,sys)}"
            )
            raise CustomException(e, sys)

    @staticmethod
    def _prepare_full_model(
        model: tf.keras.Model, classes, freeze_all, freeze_till, learning_rate
    ):
        try:
    ...
    def update_base_model(self):
        try:
            ...
            self.save_model(
                path=self.prepare_base_model_config.updated_base_model_path,
